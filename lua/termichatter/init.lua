...
M.continue = function(msg, self)
	-- Run the handler at current step, then advance
	local step = msg.pipeStep
	local pipeline = self.pipeline or M.pipeline
	local handler = pipeline[step]
	...
	if handler and type(handler) == "function" then
		msg = handler(msg, self)
		if not msg then
			return nil -- Handler filtered the message
		end
	end
	...
	return function()
		while true do
			...
			if M.isCompletion(msg) then
				-- Forward completion to next queue or output
				local nextQueue = nil
...
M.startConsumers = function(self)
	...
	-- Iterate by index to handle sparse arrays (nil holes)
	for i = 1, #(self.pipeline or {}) do
		local queue = queues[i]
		if queue then
			-- Resolve queue if it's a string
			local q = queue
			...
			if q and type(q) ~= "function" then
				...
				-- Store task reference on module
...
M.finish = function(self)
	...
	local doneMsg = vim.deepcopy(M.completion.done)
	doneMsg.pipeStep = 1
