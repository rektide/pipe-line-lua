...
describe("termichatter.line", function()
	...
	describe("ensure_mpsc", function()
		it("creates queue at position", function()
			local l = Line({ registry = registry })
			local q = l:ensure_mpsc(2)
			assert.is_not_nil(q)
			assert.is_function(q.push)
			assert.is_function(q.pop)
		end)

		it("returns same queue on repeated call", function()
			local l = Line({ registry = registry })
			local q1 = l:ensure_mpsc(3)
			local q2 = l:ensure_mpsc(3)
			assert.are.equal(q1, q2)
		end)

		it("creates independent queue per position", function()
			local l = Line({ registry = registry })
			local q1 = l:ensure_mpsc(1)
			local q2 = l:ensure_mpsc(2)
			assert.are_not.equal(q1, q2)
		end)
	end)
	...
	describe("addProcessor", function()
		...
		it("inserts at specified position", function()
			...
			l:addProcessor("b", function() end, 2)
			assert.are.equal(3, #l.pipe)
			assert.are.equal("b", l.pipe[2])
			assert.are.equal("c", l.pipe[3])
		end)

		it("creates mpsc queue with withQueue flag", function()
			local l = Line({ pipe = { "a" }, registry = registry })
			l:addProcessor("async_seg", function() end, 2, true)
			assert.is_not_nil(l.mpsc[2])
		end)
	end)
