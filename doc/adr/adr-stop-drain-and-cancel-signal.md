...
Each task transport segment owns strategy futures:
...
2. Add strategy verb methods in task transport:
   - `ensure_stopped_drain`
   - `ensure_stopped_immediate`
3. Create `stopped_drain`, `stopped_immediate`, and `stopped` futures lazily on demand.
4. Route `ensure_stopped` through `stop_type` strategy selection and await strategy-specific completion.
5. Add tests for:
   - default `stop_drain`
