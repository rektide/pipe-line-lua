...
function M.build_emits_index(registry)
	local index = {}
	for name, seg in pairs(registry.pipe) do
		local meta = resolve_meta(seg, registry)
...
--- Auto-register lattice_resolver in the registry when this module is loaded
local ok, reg = pcall(require, "termichatter.registry")
if ok and reg and reg.register then
	reg:register("lattice_resolver", {
		wants = {},
		emits = {},
		handler = M.lattice_resolver,
	})
end
