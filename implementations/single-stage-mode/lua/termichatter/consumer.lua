--- Consumer module - async message consumption from mode-backed queues
--- Single-stage-mode implementation support
local M = {}
...
M.startPipelineConsumers = function(pipeline)
	...
	for i, stage in ipairs(stages) do
		local queue = stage._queue
		if not queue and type(stage.queue) == "table" then
			queue = stage.queue
		end
		if queue then
